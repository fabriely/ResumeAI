...
export default function MeusDocumentos() {
  ...
  const fetchAnalyses = async () => {
    try {
      const response = await fetch(`/api/users/${`session.user.email`}/analyses`);
  ...
  const handleDownloadAnalysis = (imageUrl: string, fileName: string) => {
    fetch(imageUrl)
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="container mx-auto px-6 py-12 pt-24">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Meus Resumos</h1>
              ...
              return (
                <Card key={id} className="flex-1 shadow-lg border border-gray-200 bg-white rounded-xl hover:shadow-xl transition duration-300 flex flex-col">
                  ...
                  <div className="mt-4 flex justify-between pt-4 pb-2 pl-4 pr-4">
                    <Button onClick={() => handleCopySummary(parsedContent.summary_content)} size="sm" className="bg-[#007aff] text-white hover:bg-blue-700 transition rounded-full">
                      <ClipboardCopy className="w-4 h-4 mr-2" /> Copiar
                    </Button>
                    <Button onClick={() => handleDeleteSummary(id)} variant="destructive" size="sm" className="bg-[#ff2d55] text-white hover:bg-red-700 transition rounded-full p-2">
        ...
        <div className="mt-12">
          ...
          {savedImages.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
              {savedImages.map(({ id, image_url, created_at }, index) => (
                <Card key={id} className="shadow-lg border border-gray-200 bg-white rounded-xl hover:shadow-xl transition duration-300 flex flex-col">
                  <CardHeader>
                    <CardTitle className="text-lg font-semibold text-gray-900">
                      Análise {index + 1} 
                    </CardTitle>
                    <p className="text-sm text-gray-500 flex items-center">
                      <Calendar className="w-4 h-4 mr-1" /> {new Date(created_at).toLocaleString()}
                    </p>
                  </CardHeader>
                  ...
                  <div className="mt-4 flex justify-end p-4 space-x-2">
                    <Button 
                      onClick={() => handleDownloadAnalysis(image_url, `analise-${index + 1}.png`)} 
                      size="sm" 
